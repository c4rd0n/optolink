...
public class ViessmannHandlerImpl implements ViessmannHandler {
    private static Logger log = LoggerFactory.getLogger(ViessmannHandlerImpl.class);
    private ViessmannProtocol viessmannProtocol;

    ViessmannHandlerImpl(Config config, OptolinkInterface optolinkInterface) {
    ...
    public synchronized Optional<String> setValue(Telegram telegram, String value) {
        ...
        switch (telegram.getType()) {
            ...
            case Telegram.TIMER:
                if (!formatTime(value, buffer)) return Optional.empty();
                locValue = 9;
                break;
            default:
    ...
    private boolean formatTime(String value, byte[] buffer) {
        // Receiving a string of format: "On:--:--Off:--:--On:--:--Off:--:--On:--:--Off:--:--On:--:-- Off:--:--"
